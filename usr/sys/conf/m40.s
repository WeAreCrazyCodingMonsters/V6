...
trap:
/*
*将 PSW复制到距栈顶部两个字长的位置。因为 PSW包含着与陷入种类相关的信息，
*因此需要马上保存
*/
	mov	PS,-4(sp)
//检查​nofault​，如果已设置了​nofault​，则跳转到第 12行
	tst	nofault
	bne	1f
/*
*如果未设置​nofault​，则保存 SR0和 SR2的当前状态，初始化 SR0之后使用​jsr​
*指令跳转到​call1​的位置。​r0​被压入栈，它保存着​trap()​的地址
*/
	mov	SSR0,ssr
	mov	SSR2,ssr+4
	mov	$1,SSR0
...
1:
/*
*如果已设置​nofault​，则初始化 SR0，然后将保存在栈顶部被中断的进程的 pc值
*用​nofault​的值替换
*/
	mov	$1,SSR0
	mov	nofault,(sp)
/*
*使用​rtt​指令恢复栈顶部的 PSW和 pc。将 pc设置为​nofault​的值，控制权转交给
*由​nofault​指定的陷入处理函数。陷入处理函数通过​rts​指令终止运行。栈顶部保存着从
*触发陷入的函数返回时的地址，因此将返回至该地址
*/
	rtt
...
call1:
	...
	bic	$340,PS
/*
*跳转至第 25行。从此处开始的处理与中断处理时相同。因为 r0保存着​trap()​的地
*址，所以可以通过第 32行或第 46行的​jsr​指令跳转到​trap()​的位置。
*/
	br	1f
...
1:
	mov	r1,-(sp)
/*
*将被中断的进程的​sp​压入栈。​mfpi​指令用来将 PSW表示的前一模式的虚拟地址空
*间的值压入当前模式的地址空间的栈顶部。​sp​和其他的通用寄存器的不同之处在于存在两
*套，分别供用户模式和内核模式使用。内核进程在访问用户进程的 ​sp​时需要使用特殊的
*指令。
*/
	mfpi	sp
//将在第 24行压入栈的 PSW再次压入栈
	mov	4(sp),-(sp)
//将位于栈顶部的 PSW的除低位 5比特之外的部分清 0。
	bic	$!37,(sp)
//检查被中断的进程的模式，如果为内核模式则跳转到第 44行。
	bit	$30000,PS
	beq	1f
/*
*如果被中断的进程为用户进程，则使用​jsr​指令跳转到​r0​指向的地址。然后将当前
*pc保存于栈中，作为从中断处理函数返回时的地址
*/
	jsr	pc,*(r0)+
2:
//从中断处理函数返回后进行的处理。将处理器优先级设置为 7防止发生中断。
	bis	$340,PS
//如果未设置标志变量​runrun​A，则跳转到第 40行。
	tstb	_runrun
	beq	2f
/*
*如果设置了标志变量​runrun​，则将处理器优先级重置为 0。由于与执行进程（被
*中断的进程）相比存在执行优先级更高的进程，因此执行​swtch()​切换执行进程。
*/
	bic	$340,PS
//　当此进程再次执行时，返回第 33行的位置再次检查标志变量​runrun​
	jsr	pc,_swtch
	br	2b
2:
/*
*如果未设置标志变量​runrun​，被中断的进程将再次获得控制权。保存于栈中的经过
*掩码的 PSW会被忽略。
*/
	tst	(sp)+
/*
*恢复保存于栈中的被中断的进程的 sp。​mtpi​指令用来复制前一处理器模式（由 PSW
*表示）虚拟地址空间中栈顶部的值。
*/
	mtpi	sp
//　跳转到第 48行
...
1:
/*
*被中断的进程为内核进程时，将前一模式设置为用户模式，然后跳转到由​r0​指
*向的中断处理函数的位置。
*/
	bis	$30000,PS
	jsr	pc,*(r0)+
/*
*从中断处理函数返回后，忽略栈顶部的经过掩码的 PSW以及被中断的进程的 sp。之
*后的处理与用户进程被中断时的处理相同
*/
	cmp	(sp)+,(sp)+
2:
//恢复保存在栈内的​r1​和​r0​，忽略 PSW
	mov	(sp)+,r1
	tst	(sp)+
	mov	(sp)+,r0
/*
*　执行​rtt​指令，恢复保存在栈中的被中断的进程的 PSW和 pc，并继续被中断的进程
*的处理。​rtt​指令用来将栈顶部两个字长的数据设置到 PSW和 pc中。
*/
	rtt
