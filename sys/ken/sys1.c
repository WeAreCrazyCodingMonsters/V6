...
​	/*​正当性检查​*/
	ip = namei(&uchar, 0);
	/*
	* 通过用户作为参数指定的程序执行文件的路径，取得​inode[]​中对应的该文件的元素。
	* 如果用户指定了不存在的文件路径，或是不具备访问该文件的权限时，​namei()​将返回​NULL​
	*/
	if(ip == NULL)
		return;
	
	/*
	* 确认正在执行的​exec()​的进程数小于​NEXEC​。
	* 如果大于或等于​NEXEC​则调用sleep()​进入睡眠状态，直到正在执行的​exec()​的进程数小于或等于​NEXEC​。
	* 通过此处的检查后递增​execnt​的值，表示正在执行​exec()​的进程数又增加了 1个。
	*/
	...
	/* 
	* 执行​getblk()​取得尚未被分配给其他块设备的块设备缓冲区，并将其作为处理参数时的工作内存。
	* 大概开发人员认为与通过​malloc()​和​mfree()​分配内存相比，这种做法更简单。
	*/
	bp = getblk(NODEV);

	// 执行​access()​检查程序文件的执行权限，同时确认该文件不是特殊文件。				
	...
	/* 
	* 将传递给程序的参数存入刚才取得的缓冲区。
	* 首先将​cp​设定为缓冲区数据区域的地址，然后将​na​（参数的数量）和​nc​（参数的总字节数）清 0。
	*/
	cp = bp->b_addr;
	na = 0;
	nc = 0;
	/* 
	* 依次处理用户传递给程序的参数。
	* ​fuword()​是从上一模式所示的虚拟地址空间向当前模式所示的虚拟地址空间复制的 1个字长的数据。
	* ​u.u_arg[1]​此时为系统调用​exec​的第 2个参数的地址。
	*/

	while(ap = fuword(u.u_arg[1])) {
		na++;
		// ​fuword()​处理失败时返回 -1，此时跳转到​bad​进行错误处理。
		if(ap == -1)
			goto bad;
		// 将​u.u_arg[1]​增加 1个字以便访问下一个参数。
		u.u_arg[1] =+ 2;
		/* 
		* 将参数以字节为单位依次存入缓冲区,将​ap​设定为指向该参数的指针。
		* fubyte()​与​fuword()​相似，但是处理单位为 1个字节。
		* 参数以字节为单位存入缓冲区。
		* fubyte()​发生错误时返回 -1，此时跳转到​bad​进行错误处理。
		* 每处理 1个字节，​nc​都会加 1。当这个​while​处理结束时，​nc​的值等于参数的总字节数。
		* 因为缓冲区的长度只有 512字节，当​nc​大于 510时将认为参数的长度过长，此时跳转到​bad​进行错误处理。
	...
	// 当​nc​的值为奇数时，向缓冲区中追加 1个字节使数据长度成为偶数。因为系统希望以字为单位进行处理。
	if((nc&1) != 0) {
		*cp++ = 0;
		nc++;
	}
	...
	u.u_offset[1] = 0;
	u.u_offset[0] = 0;
	// 读取程序文件的文件头。​readi()​是用于读取文件内容的函数，其参数由​user​结构体指定。
	u.u_segflg = 1;
	readi(ip);
	u.u_segflg = 0;
	// 如果​readi()​在执行中发生错误，将错误代码保存至​u.u_error​，并跳转到bad​进行错误处理。
	if(u.u_error)
		goto bad;
	sep = 0;
	/* 
	* 根据魔术数字进行分支处理。魔术数字为 0407时不使用代码段，而是将代码和数据一并读取至数据区域。
	* 将​u.u_arg[2]​设置为代码长度和数据长度之和，将​u.u_arg[1]​清 0。
	...
	*/
	...
	// 如果代码的长度（​u.u_arg[1]​）不为 0，该程序文件又被作为数据文件打开，则进行错误处理。
	if(u.u_arg[1]!=0 && (ip->i_flag&ITEXT)==0 && ip->i_count!=1) {
		u.u_error = ETXTBSY;
		goto bad;
	}
	// 计算代码段和数据区域的长度。段的长度以 64字节为单位进行管理。
	ts = ((u.u_arg[1]+63)>>6) & 01777;
	ds = ((u.u_arg[2]+u.u_arg[3]+63)>>6) & 01777;
	// 执行​estabur()​更新用户 APR和用户空间A。​SSIZE​表示栈区域的初始长度，被定义为 20（×64字节）。
	if(estabur(ts, ds, SSIZE, sep))
		goto bad;


	/*​读取程序执行文件的代码部分​*/
	// 从此处开始进行代码段和数据段的初始化处理。首先将用于统计的变量清 0。
	u.u_prof[3] = 0;
	/* 
	* 执行​xfree()​释放当前使用的代码段。如果在此之前依次执行了​fork​和​exec​，
	* 那么到此为止就终于告别了父进程所使用的代码段。
	*/
	xfree();
	// 执行​expand()​将数据段缩小为与​user​结构体相同的长度。
	expand(USIZE);
	// 执行​xalloc()​分配代码段使用的内存。如果未使用代码段，则在​xalloc()​中不做任何处理。
	xalloc(ip);
	c = USIZE+ds+SSIZE;
	/* 
	* 执行​expand()​确保数据区域。
	* 由于位于​user​结构体之后的区域留有过去的数据，因此需要将其清 0。
	*/
	expand(c);
	while(--c >= USIZE)
	...
	/*​读取程序执行文件的数据部分​*/
	// 执行​estabur()​，将进程的数据区域的起始位置变更至虚拟地址为 0的位置。
	estabur(0, ds, 0, 0);
	u.u_base = 0;
	...
	/*
	* 执行​estabur()​最终确定用户空间。
	* 魔术数字为 0410时，通过​estabur()​设定用户 APR，
	* 对虚拟地址空间进行如下变更：将代码段起始地址调整至虚拟地址为 0处，
	* 数据区域起始地址以 8KB为边界对齐。
	*/
	u.u_tsize = ts;
	u.u_dsize = ds;
	...
	​/*​将参数压入栈​*/
	// 从这里开始将保存于缓冲区的参数转存至栈区域。首先将 cp设定为缓冲区的数据区域的地址。
	cp = bp->b_addr;
	/* 
	* 计算栈指针的地址，并设定用户进程的 sp。
	* 请注意，因为栈区域的起始地址为​0xffff​，换算成​int​型将为负数。
	*/
	ap = -nc - na*2 - 4;
	u.u_ar0[R6] = ap;
	
	/* 
	* 将​na​（参数的数量）的值设定至栈指针指向的用户空间地址。
	* suword()​是从当前模式的虚拟地址空间向前一模式的虚拟地址空间复制的数据。
	*/
	suword(ap, na);
	c = -nc;// 将​c​设定为保存参数的用户空间地址。
	while(na--) {
		suword(ap=+2, c);
		do
			subyte(c++, *cp);
